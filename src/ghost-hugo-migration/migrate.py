...
# -*- coding: utf-8 -*-
'''
Script derived version from http://caveconfessions.com/ghost-to-hugo/
...
def migrate(filepath):
    ...
    with codecs.open(filepath, 'r', 'utf-8-sig') as fp:
        ghost = json.load(fp)
    ...
    for post in data['posts']:
        ...
        target = "{}.md".format(post["slug"])

        aliases = ["/{}/".format(post["slug"])]

        print("Migrating '{}' to {}".format(post["title"], target))
